...
class Db {
  ...
  createHook (data, callback) {
    ...
    let tasks = co.wrap(function * () {
      ...
      if (campaign) {
        ...
        let findbyemail = yield r.db(db).table('historicEmail')
            .filter({email:data.email, campaingId: data.campaingId}).count().run(conn)
        if (findbyemail > 0) {
          let old_data = yield r.db(db).table('historicEmail').filter({email:data.email, campaingId: data.campaingId}).run(conn)
          old_data =  yield old_data.toArray()
          if (data.event === 'open') {
            if (old_data[0].event === 'click' && data.event !== 'open') {
              yield r.db(db).table('historicEmail').filter({email:data.email, campaingId: data.campaingId})
              .update({event: data.event}).run(conn)
            }
          } else {
              yield r.db(db).table('historicEmail').filter({email:data.email, campaingId: data.campaingId})
              .update({event: data.event}).run(conn)
  ...
  customFind (collection, id, index, row, params, filter, callback) {
    ...
    let tasks = co.wrap(function * () {
      let conn = yield connection
      let query = yield r.db(db).table(collection).getAll(id, {index: index}).filter(r.row(row).match(params)).filter(filter).limit(100).run(conn)
      try {
  ...
  filterCamapings (collection, row, params, filter, callback){
    ...
    let db = this.db
      ...
      let tasks = co.wrap(function * () {
        let conn = yield connection
        let query = yield r.db(db).table(collection).filter(r.row(row).match(params)).filter(filter)
  ...
  graphEmailsbyCampaign (id, callback) {
    ...
    let tasks = co.wrap(function * () {
      ...
      let report = yield r.db(db).table('historicEmail').getAll(id,{index: 'campaingId'})
      .run(conn)
      report = yield report.toArray()
      for (let i = 0; i < report.length; i++) {
        switch (report[i].event) {
          case 'open':
            data.open += 1
            break
          case 'click':
  ...
  getReportEmailByDay (user, initDay, lastDay ,callback) {
    ...
    let tasks = co.wrap(function * () {
      try {
        ...
        let report = yield r.db(db).table('historicEmail').between(initDay, lastDay,{index: 'date'})
         .filter({userId: user}).run(conn)
        report = yield report.toArray()
        for (let i = 0; i < report.length; i++) {
          switch (report[i].event) {
            case 'open':
  ...
  campaignSended (user, callback) {
    let db = this.db
    let connection = this.connection
    let tasks = co.wrap(function * () {
      try {
        let conn = yield connection
        let smsSended = yield r.db(db).table('historicSms').getAll(user, {index: 'userId'}).count().run(conn)
        let emailSended = yield r.db(db).table('historicEmail').getAll(user,{index: 'userId'}).count().run(conn)
        let data = {
          sms: smsSended,
          email: emailSended
        }
