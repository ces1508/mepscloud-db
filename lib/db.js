...
class Db {
  ...
  all (collection, key, index,  skip, callback) {
    let db = this.db
    if (!skip) skip = 0
    let limit = 100
    ...
    let tasks = co.wrap(function * () {
      let conn = yield connection
      let result = yield r.db(db).table(collection).getAll(key, {index: index}).skip(skip).limit(limit).run(conn)
      let count = yield r.db(db).table(collection).getAll(key, {index: index}).count().run(conn)
      try {
  ...
  allCampaing (collection ,userId, skip, callback) {
    ...
    let tasks = co.wrap(function * () {
      ...
      let result = yield r.db('mepscloud').table(collection).getAll(userId,
        {index: 'userId'}).eqJoin('databaseId',r.db('mepscloud').table('databases')).without([{right:"id"}]).zip().skip(skip).limit(limit).run(conn)
  ...
  updateNumberContacts (id, callback) {
    ...
    let tasks = co.wrap(function * () {
      let conn = yield connection
      let count = yield r.db('mepscloud').table('contacts').getAll(id, {index: 'databaseId'}).count().run(conn)
      let updated = yield r.db('mepscloud').table('databases').get(id).update({contacts: count}).run(conn)
      return Promise.resolve(updated)
    })
    return Promise.resolve(tasks()).asCallback(callback)
  }
  customFind (collection, row, params, filter, callback) {
    let db = this.db
    let connection = this.connection
    let result = []
    filter = filter || {}
    let tasks = co.wrap(function * () {
      let conn = yield connection
       let query = yield r.db('mepscloud').table(collection).filter(r.row(row).match(params)).filter(filter).limit(100).run(conn)
      try {
  ...
  findContactsByNumber (id, phone, callback) {
    let db = this.db
    let connection = this.connection
    let result = []
    let tasks = co.wrap(function * () {
      ...
      let query = yield r.db('mepscloud').table('contacts').filter({databaseId: id, phone: number}).run(conn)
      try {
        result = yield query.toArray()
      } catch (e) {
        return Promise.reject(new Error(e.message))
