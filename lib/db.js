...
class Db {
  ...
  createHook (data, callback) {
    ...
    let tasks = co.wrap(function * () {
      let conn = yield connection
      let campaign = yield r.db(db).table('campaingEmails').get(data.campaingId).run(conn)
      if (campaign) {
  ...
  graphEmailsbyCampaign (id, callback) {
    ...
    let tasks = co.wrap(function * () {
       let data = {
        ...
        data.dropped = yield r.db(db).table('historicEmail').getAll(id,{index: 'campaingId'}).filter({event: 'dropped'}).count().run(conn)
        data.spam = yield r.db(db).table('historicEmail').getAll(id,{index: 'campaingId'}).filter({event: 'spamreport'}).count().run(conn)
    ...
    let tasks = co.wrap(function * () {
      try {
       ...
       let data = {
        ...
        data.dropped = yield r.db(db).table('historicEmail').between(initDay, lastDay,{index: 'date'}).filter({userId: user, event: 'dropped'}).count().run(conn)
        data.spam = yield r.db(db).table('historicEmail').between(initDay, lastDay,{index: 'date'}).filter({userId: user, event: 'spamreport'}).count().run(conn)
