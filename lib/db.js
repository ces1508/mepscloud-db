...
class Db {
   ...
   let connection = this.connection
    let tasks = co.wrap(function * () {
      ...
      if (findbyemail > 0) {
        ...
        if (old_data[1].event === 'click' && data.event !== 'open') {
  ...
  getReportSmsByDay (user, initDay, lastDay ,callback ) {
    ...
    let tasks = co.wrap(function * () {
      try {
        let conn = yield connection
        let report = yield r.db(db).table('statistics').between(initDay, lastDay,{index: 'date'})
          .filter({userId: user,type: 'sms'}).without('campaingId', 'userId','id').run(conn)
        ...
        data.date = initDay
          for (let i = 0; i < report.length; i++) {
            data.send += report[i].send
            data.failed += report[i].failed
            data.date = report[i].date
          }
        return Promise.resolve(data)
    ...
    let tasks = co.wrap(function * () {
      try {
        let conn = yield connection
        let report = yield r.db(db).table('statistics').between(initDay, lastDay,{index: 'date'})
          .filter({userId: user,type: 'email'}).without('campaingId', 'userId','id').run(conn)
