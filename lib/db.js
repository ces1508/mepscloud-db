...
class Db {
    ...
    let setup = co.wrap(function * () {
      ...
      if (config) {
        ...
        if (dbTables.indexOf('historicEmail') === -1) {
          ...
          yield r.db(db).table('historicEmail').indexWait('userId').run(conn)
        ...
        if (dbTables.indexOf('statistics') === -1) {
          ...
          yield r.db(db).table('statistics').indexWait('campaingId').run(conn)
  ...
  allTemplateEmails (userId, skip, callback) {
    ...
    let tasks = co.wrap(function * () {
      let conn = yield connection
      let result = r.db(db).table('templateEmails').getAll(userId, {index: 'userId'})
